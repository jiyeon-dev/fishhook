...
- Jira 이미지·동영상 첨부를 표시한다. 이미지는 `<img src>` 직접 로드 + 클릭 전체화면, 동영상은 `media-loader.js`로 background fetch 후 재생한다.
...
- 이미지는 `<img src>`로 Jira URL을 직접 표시하고, 클릭 시 전체화면(라이트박스)으로 확대한다.
...
FISHHOOK_FETCH_JIRA_ATTACHMENT
